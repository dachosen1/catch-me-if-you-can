...
st.markdown(
    "The financial services sector is one of the first in line to pick up the benefits of AI and Machine "
...
payment_type = st.sidebar.selectbox(
    label="Type", options=["TRANSFER", "CASH-IN", "CASH-OUT", "DEBIT", "PAYMENT"]
)
...
flagged_fraud = st.sidebar.selectbox(
    label="Flags illegal attempts to transfer more than 200.000 in a single "
    "transaction",
    options=["true", "false"],
)

flagged_dict = {"true": 1, "false": 0}
